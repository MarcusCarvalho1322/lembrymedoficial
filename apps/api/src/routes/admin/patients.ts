...
router.get(
  ...
  try {
    ...
    // FIX-10: Eliminar N+1 — 3 subqueries correlacionadas substituídas por
    // LEFT JOINs com agregação condicional. Uma única query, custo O(n) → O(1).
    // Filtro de data usa `todayBRT` (não CURRENT_DATE/UTC) para evitar bug de
    // cruzamento de meia-noite entre 21h e 23h59 BRT.
    let query = sql`
      SELECT
        p.id, p.full_name, p.phone, p.email, p.onboarding_step, p.is_active,
        p.created_at,
        COUNT(DISTINCT m.id)  FILTER (WHERE m.is_active = true)                      as med_count,
        COUNT(DISTINCT mc.id) FILTER (WHERE mc.date = ${todayBRT}::date
                                        AND mc.confirmation_status = 'confirmed')    as confirmed_today,
        COUNT(DISTINCT mc.id) FILTER (WHERE mc.date = ${todayBRT}::date
                                        AND mc.confirmation_status != 'confirmed')   as missed_today,
        MAX(rl.sent_at)                                                              as last_activity
      FROM patients p
      LEFT JOIN medications            m  ON m.patient_id  = p.id
      LEFT JOIN medication_confirmations mc ON mc.patient_id = p.id
    ...
    query = sql`${query} GROUP BY p.id, p.full_name, p.phone, p.email, p.onboarding_step, p.is_active, p.created_at
...
router.patch(
  ...
  try {
    ...
    // Invalida cache: mudanças no paciente podem afetar quem recebe lembretes
    // (ex: is_active alterado, onboarding_step alterado, telefone alterado)
    invalidateAndSync().catch(() => {}); // best-effort, não bloqueia resposta
...
router.delete(
  ...
  try {
    ...
    // Invalida cache: paciente deletado não deve mais receber lembretes
    invalidateAndSync().catch(() => {}); // best-effort, não bloqueia resposta
