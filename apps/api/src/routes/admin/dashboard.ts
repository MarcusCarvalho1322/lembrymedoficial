  ...
  try {
    ...
    const kpisResult = await db.execute(sql`
      SELECT
        ...
        -- Ticket médio calculado da base real (assinaturas ativas)
        -- Evita hardcoded que nunca reflete mudanças de plano.
        (SELECT COALESCE(ROUND(AVG(amount_cents) / 100.0, 2), 0)
    ...
    const ticketMedio         = Number(row.ticket_medio_real)     || 0;
    ...
    res.json({
      ...
      newPatients30d,
      onboardingPending,
      ticketMedio, // calculado da base real — não mais hardcoded
