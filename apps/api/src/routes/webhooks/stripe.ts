...
router.post('/webhook/stripe', async (req, res) => {
  ...
  try {
    event = stripe.webhooks.constructEvent(
      req.body, sig, env.STRIPE_WEBHOOK_SECRET
    );
  } catch (err: any) {
  ...
  logger.info('Processing checkout.session.completed (new patient)', { name, phone: phone.substring(0, 8) + '****', sessionId: session.id });
  ...
  try {
    ...
    logger.info('Patient upserted', { patientId, phone: phone.substring(0, 8) + '****' });
...
async function handleRenewal(
  ...
  try {
    ...
    // Invalida e re-sincroniza: paciente e meds acabaram de ser reativados
    ...
    logger.info('Mensagem de renovação enviada', { patientId, phone: patient.phone.substring(0, 8) + '****' });
