...
export function startOnboardingNudgeWorker() {
  ...
  cron.schedule('0 * * * *', async () => {
    ...
    try {
      ...
      // Bug pré-existente pós-auditoria: `ANY(${STUCK_STEPS}::text[])` falhava com
      // "cannot cast type record to text[]" porque o template sql passa o array
      // como ROW. Usamos IN (...) com join explícito de parâmetros.
      const stuckStepsSql = sql.join(
        STUCK_STEPS.map((s) => sql`${s}`),
        sql`, `,
      );
      const stuckPatients = await db.execute(sql`
        SELECT
          ...
          p.updated_at
        FROM patients p
        WHERE p.onboarding_step::text IN (${stuckStepsSql})
