...
function LoginScreen({ onLogin }: { onLogin: (t: string) => void }) {
  ...
  const submit = async () => {
    ...
    try {
      // Cookie httpOnly é setado pelo backend; o token também volta no body
      // para retrocompat. Mantemos em memória (estado React), NÃO em
      // localStorage — defesa contra XSS exfiltrar o token.
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Tenta /admin/me para validar cookie httpOnly. Se válido, marca como
    // autenticado SEM precisar do token em memória (fluxo cookie-only).
    // Fallback: tenta migrar token legado do localStorage (será removido em build futuro).
    (async () => {
      try {
        await adminMe();
        setToken('__cookie__'); // sentinela: estamos autenticados via cookie
      } catch {
        const legacy = typeof window !== 'undefined' ? localStorage.getItem('lembrymed_token') : null;
        if (legacy) {
          // Limpa o legado e exige re-login (cookie httpOnly é a única fonte de verdade).
          localStorage.removeItem('lembrymed_token');
        }
      } finally {
        setReady(true);
      }
  ...
  const logout = async () => {
    await adminLogout();
    if (typeof window !== 'undefined') localStorage.removeItem('lembrymed_token');
    setToken(null);
  };
