...
import { BarChart, Gauge } from '@/components/admin/charts';
import type { DashboardResponse, NewSubscribersDay } from '@lembrymed/shared';
...
export default function AdminDashboard() {
  ...
  const [newSubs, setNewSubs] = useState<NewSubscribersDay[]>([]);
  ...
  useEffect(() => {
    if (!token) return;
    const load = () => {
      api<DashboardResponse>('/admin/dashboard', { token }).then(setData).catch(e => setError(e.message));
      api<NewSubscribersDay[]>('/admin/dashboard/new-subs', { token }).then(setNewSubs).catch(() => {});
    };
    load();
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <div style={{ background: '#0A0A0A', border: '1px solid rgba(255,255,255,.04)', borderRadius: 14, padding: 24 }}>
              ...
              <div style={{ fontFamily: "'Cinzel',serif", fontSize: 22, fontWeight: 700, color: '#F0EDE8', marginBottom: 16 }}>
                {newSubs.reduce((s, d) => s + Number(d.count), 0)}
              </div>
              <BarChart data={newSubs.map((d) => ({
                label: d.day,
                value: Number(d.count),
              }))} />
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                {newSubs.slice(-7).map((d, i) => (
            ...
            <div style={{ background: '#0A0A0A', border: '1px solid rgba(255,255,255,.04)', borderRadius: 14, padding: 24 }}>
              ...
              {[
                ...
                // FIX (Onda 4): usa o ticket calculado pelo backend (média da base
                // de assinaturas ativas) em vez de hardcoded R$ 149.
                {
                  label: 'Ticket médio',
                  value: `R$ ${(data.ticketMedio || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
                  icon: '◉',
                },
