...
export const patients = pgTable('patients', {
  ...
  index('idx_patients_session').on(table.agentSessionId),
]);
...
export const familyContacts = pgTable('family_contacts', {
  ...
  index('idx_family_patient').on(table.patientId),
]);
...
export const subscriptions = pgTable('subscriptions', {
  ...
  index('idx_subs_patient').on(table.patientId),
  index('idx_subs_status').on(table.status),
  index('idx_subs_expires').on(table.expiresAt),
  index('idx_subs_stripe').on(table.stripeCustomerId),
]);
...
export const medications = pgTable('medications', {
  ...
  index('idx_meds_patient').on(table.patientId),
]);
...
export const reminderLogs = pgTable('reminder_logs', {
  ...
  index('idx_reminders_scheduled').on(table.scheduledFor, table.status),
  index('idx_reminders_patient').on(table.patientId, table.scheduledFor),
]);
...
export const medicationConfirmations = pgTable('medication_confirmations', {
  ...
  index('idx_conf_patient_date').on(table.patientId, table.date),
  uniqueIndex('idx_conf_unique').on(
...
export const messageLogs = pgTable('message_logs', {
  ...
  index('idx_msgs_patient').on(table.patientId, table.createdAt),
  index('idx_msgs_phone').on(table.phone, table.createdAt),
]);
...
export const familyAlertLogs = pgTable('family_alert_logs', {
  ...
  index('idx_family_alerts_dedup').on(table.patientId, table.familyContactId, table.date, table.status),
]);
...
export const consentLogs = pgTable('consent_logs', {
  ...
  index('idx_consent_phone').on(table.phone),
]);
...
export const adminAuditLogs = pgTable('admin_audit_logs', {
  ...
  index('idx_audit_admin_created').on(table.adminEmail, table.createdAt),
  index('idx_audit_patient').on(table.patientId, table.createdAt),
