...
/* ─── Login + Recuperação ──────────────────────────────────── */
type ViewMode = 'login' | 'forgot-email' | 'reset-code';
...
function LoginScreen({ onLogin }: { onLogin: (t: string) => void }) {
  ...
  const submitForgot = async () => {
    ...
    try {
      ...
      setInfo(
        res.maskedPhone
          ? `Código enviado para o WhatsApp ${res.maskedPhone}. Válido por 10 minutos.`
          : 'Se o e-mail existir, um código foi enviado.',
      );
      setView('reset-code');
    } catch (e: any) {
      setError(e.message || 'Falha ao pedir código. Tente novamente.');
    } finally { setLoading(false); }
  };
  ...
  const submitReset = async () => {
    ...
    try {
      ...
      setInfo('Senha alterada! Voltando ao login...');
      setTimeout(() => {
        setView('login');
      ...
      setError(e.message || 'Falha ao redefinir senha. Verifique o código.');
    } finally { setLoading(false); }
  };
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .link-btn{background:transparent;border:none;color:#B87333;font-family:'Cormorant Garamond',serif;font-size:13px;cursor:pointer;padding:6px 0;transition:color .2s;}
        .link-btn:hover{color:#D4A853;}
        ...
        <div style={{ background: '#0A0A0A', border: '1px solid rgba(255,255,255,.04)', borderRadius: 16, padding: 36 }}>
          ...
          {view === 'login' && (
            <>
              ...
              <button className="login-btn" onClick={submitLogin} disabled={loading} style={{ marginTop: 24 }}>
                {loading ? 'VERIFICANDO...' : 'ENTRAR'}
              </button>
              <div style={{ textAlign: 'center', marginTop: 18 }}>
                <button className="link-btn" onClick={() => { setView('forgot-email'); reset(); }}>
                  Esqueci minha senha
                </button>
              </div>
            </>
          ...
          {view === 'forgot-email' && (
            <>
              <p style={{ fontFamily: "'Cinzel',serif", fontSize: 10, letterSpacing: 6, color: '#B87333', textTransform: 'uppercase' as const, marginBottom: 10 }}>
                RECUPERAR SENHA
              </p>
              <p style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontSize: 13, color: '#998E82', marginBottom: 20, lineHeight: 1.5 }}>
                Digite seu e-mail. Enviaremos um código de 6 dígitos no seu WhatsApp cadastrado.
              </p>
              <input className="login-input" type="email" placeholder="E-mail"
                value={email} onChange={e => setEmail(e.target.value)}
              ...
              <button className="login-btn" onClick={submitForgot} disabled={loading || !email} style={{ marginTop: 20 }}>
                {loading ? 'ENVIANDO...' : 'ENVIAR CÓDIGO'}
              </button>
              <div style={{ textAlign: 'center', marginTop: 18 }}>
                <button className="link-btn" onClick={() => { setView('login'); reset(); }}>
                  ← Voltar ao login
                </button>
              </div>
            </>
          ...
          {view === 'reset-code' && (
            <>
              ...
              {info && (
                <div style={{ marginBottom: 16, padding: '10px 14px', background: 'rgba(91,184,91,.1)', border: '1px solid rgba(91,184,91,.3)', borderRadius: 6, color: '#5BB85B', fontSize: 13, fontFamily: "'Cormorant Garamond',serif" }}>
                  {info}
                </div>
              )}
              <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 12 }}>
                <input className="login-input" type="text" placeholder="Código de 6 dígitos"
                  value={code} onChange={e => setCode(e.target.value.replace(/\D/g, '').slice(0, 6))}
                  maxLength={6} inputMode="numeric" />
                <input className="login-input" type="password" placeholder="Nova senha (mín. 8 caracteres)"
              ...
              <div style={{ textAlign: 'center', marginTop: 18, display: 'flex', justifyContent: 'space-between' }}>
                <button className="link-btn" onClick={() => { setView('forgot-email'); reset(); }}>
                  ← Reenviar código
                </button>
                <button className="link-btn" onClick={() => { setView('login'); reset(); }}>
