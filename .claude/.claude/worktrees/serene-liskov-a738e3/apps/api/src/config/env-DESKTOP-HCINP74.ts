...
const envSchema = z.object({
  ...
  // ── Anthropic Claude ──────────────────────────────────────────────────────
  ANTHROPIC_API_KEY: z.string().trim().startsWith('sk-ant-'),
  ...
  ADMIN_PASSWORD_HASH: z.string().min(40), // bcrypt hash é ~60 chars — usado como fallback/bootstrap
  /**
   * Telefone admin para receber código de recuperação de senha via WhatsApp.
   * Opcional no boot (pode estar só no banco após seed) — mas obrigatório
   * em produção para que "Esqueci minha senha" funcione.
   */
