...
async function processWebhookPayload(payload: any): Promise<void> {
  ...
  await db.insert(messageLogs).values({
    phone,
    direction: 'inbound',
    content:   text || (audioUrl ? '[audio]' : '[media]'),
    mediaType: audioUrl ? 'audio' : imageUrl ? 'image' : 'text',
  ...
  const wantsMedUpdate    = MED_UPDATE_REGEX.test(text)    || (!!imageUrl && !isInFamilyMode);
  const wantsFamilyUpdate = FAMILY_UPDATE_REGEX.test(text) || isInFamilyMode;

  if (isInMedMode || (!isInFamilyMode && wantsMedUpdate)) {
    await handleMedUpdateConversation(patient, text, imageUrl, isInMedMode);
    return;
  }
...
async function handleOnboardingConversation(
  ...
  logger.info('Chamando Claude Sonnet para onboarding', {
    patientId: patient.id, step, historyLen: history.length,
  });
...
- Se receber foto de receita, diga que "viu a receita" e peça confirmação dos medicamentos
...
async function handleMedUpdateConversation(
  ...
  logger.info('Chamando Claude Sonnet para med_update', {
    patientId: patient.id, currentMedCount: currentMeds.length,
  });
